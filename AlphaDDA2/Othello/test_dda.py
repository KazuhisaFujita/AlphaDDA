...
from AlphaDDA2 import A_MCTS as DDA
from nn import NNetWrapper
from nn_dropout import NNetWrapper as NNetWrapper_DP
import time
...
class Cal:
    def __init__(self, num_mean = 1, A = 1000, X0 = 0.0):
        self.num_games = 50
        ...
        self.net_dp  = NNetWrapper_DP()
        self.net_dp.load_checkpoint()
    ...
    def dda(self, g, count, states):
        amcts = DDA(game = g, net = self.net_dp, num_mean = self.num_mean, X0 = self.X0, A = self.A, states = states)
    ...
    def Cal(self):
        ...
        for i in self.players:
            print("first:", self.num_mean, self.A, self.X0, i, win_lose_first[i][0], win_lose_first[i][1], self.num_games - win_lose_first[i][0] - win_lose_first[i][1])
        ...
        for i in self.players:
            print("second:", self.num_mean, self.A, self.X0, i, win_lose_second[i][0], win_lose_second[i][1], self.num_games - win_lose_second[i][0] - win_lose_second[i][1])

        for i in self.players:
            win_lose[i] += win_lose_second[i]

        for i in self.players:
            print("total:", self.num_mean, self.A, self.X0, i, win_lose[i][0], win_lose[i][1], self.num_games * 2- win_lose[i][0] - win_lose[i][1])

    def parallel_play(self, schedule):
        win_lose = self.Init_win(self.players)

        num_processes = 16
        devices       = ['cuda:0', 'cuda:1', 'cuda:0', 'cuda:1', 'cuda:0', 'cuda:1', 'cuda:0', 'cuda:1', 'cuda:0', 'cuda:1', 'cuda:0', 'cuda:1', 'cuda:0', 'cuda:1', 'cuda:0', 'cuda:1', 'cuda:0', 'cuda:1', 'cuda:0', 'cuda:1']
...
if __name__ == '__main__':
    ...
    A = float(sys.argv[2])
    X0 = float(sys.argv[3])
    cal = Cal(num_mean = num_mean, A = A, X0 = X0)
    cal.Cal()
