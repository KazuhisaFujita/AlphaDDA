...
class Cal_Elo_rating:
    ...
    def dda(self, g, count):
        amcts = DDA(game = g, net = self.net_dp, estimated_outcome = self.estimated_outcome, num_mean = self.num_mean, X0 = self.X0, A = self.A)
        amcts.Store_outcome()
    ...
    def Cal_elo(self):
        ...
        for i in self.players:
            print(self.num_mean, self.A, self.X0, i, self.win_lose[i][0], self.win_lose[i][1], self.num_games * 2 - self.win_lose[i][0] - self.win_lose[i][1])
...
if __name__ == '__main__':
    ...
    A = float(sys.argv[2])
    X0 = float(sys.argv[3])
    cal_elo = Cal_Elo_rating(num_mean = num_mean, A = A, X0 = X0)
    cal_elo.Cal_elo()
